...
/**
 * @brief RIT timer delay function
 * @param us Delay in milliseconds
 */
void delayMicroseconds(unsigned int us)
{
	// How many times microsecond period is bigger than cc
	uint64_t period_ratio = Chip_Clock_GetSystemClockRate() / 1000000;
	// Target number of ticks
	uint64_t compare_value = period_ratio * us;

	Chip_RIT_Disable(LPC_RITIMER); // Disable RIT
	Chip_RIT_SetCompareValue(LPC_RITIMER, compare_value); // Set compare value
	Chip_RIT_SetCounter(LPC_RITIMER, 0); // Clear counter value
	Chip_RIT_Enable(LPC_RITIMER); // Enable timer

	while(!Chip_RIT_GetIntStatus(LPC_RITIMER)); // Wait for the interrupt flag to arrive
...
/**
 * @brief Define LCD geometry
 * @param cold    Number of columns
 * @param lines   Number of lines
 * @param dotsize Size of cell
 */
...
/**
 * @brief Set position of the cursor to (row, col)
...
/**
 * @brief Set text flow from left to right
 */
...
/**
 * @brief Set text flow from right to left
 */
